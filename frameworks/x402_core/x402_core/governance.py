...
`tesoro.plugin.Governor` satisfies that, and so could anything else. Keeping it
structural is the point: if the agents imported `tesoro`, "a plugin you can install"
...
`tesoro.adapters.base.RunGuard` is now the same contract, shipped in the package. This
